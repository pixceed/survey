...
class ReActAgent:
    ...
    def _create_graph(self) -> StateGraph:
        ...
        graph_builder.add_conditional_edges(
            ...
            tools_condition
            # tools_conditionとは
            # アシスタントからの最新メッセージがツール呼び出しである場合 → ノード"tools"にルーティングする
            # アシスタントからの最新メッセージがツール呼び出しでない場合 → ノード"END"にルーティングする
        )
    ...
    def _assistant(self, state: ReActAgentState) -> dict[str, Any]:
        ...
        if len(before_messages) > 0:
            print("★"*80)
            # print(before_messages[-2])
            # print()
            # print(before_messages[-1])
...
def main():
    ...
    # query = "3と4を足す。その出力に2を掛ける。 さらにその出力を5で割る。次に小数点第一位で四捨五入する。最後にその数を100倍する。"
